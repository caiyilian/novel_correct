...
def process_single_file(input_file: str, args, my_chunk_sizes, OUTPUT_DIR_V1, OUTPUT_DIR_V2, OUTPUT_DIR_V3, OUTPUT_DIR_V4, OUTPUT_DIR_V5):
    ...
    if args.pipeline:
        ...
        # --- Stage 1: 暴力扫描 ---
        debug_print(f"▶️ [Stage 1] 执行全局暴力扫描...", debug=True)
        ...
        if not os.path.exists(stage1_output):
            debug_print(f"\n❌ Stage 1 产出文件 {stage1_output} 不存在，流水线终止！", debug=True)
            return
            
        debug_print(f"\n▶️ [Stage 2] 执行规则交替修复...", debug=True)
        correct_single_volume_rule(
            input_file_path=stage1_output,  # 这里的输入是 Stage 1 的输出
